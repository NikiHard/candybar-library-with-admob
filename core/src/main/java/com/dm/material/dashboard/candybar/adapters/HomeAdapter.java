        ...
        if (holder.getItemViewType() == TYPE_HEADER) {
            ...
            if (mHomes.get(finalPosition).getIcon() != -1) {
                ...
                if (mHomes.get(finalPosition).getType() == Home.Type.DIMENSION) {
                    int id = getResourceId(mContext, mHomes.get(finalPosition).getTitle());
                    contentViewHolder.autoFitTitle.setCompoundDrawablesWithIntrinsicBounds(0, 0, id ,0);
            ...
            String name = IconsHelper.replaceName(mContext, true, mHomes.get(finalPosition).getTitle());
            contentViewHolder.autoFitTitle.setText(name);
